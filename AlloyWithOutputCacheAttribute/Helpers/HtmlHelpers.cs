...
using AlloyWithOutputCacheAttribute.Business;
...
namespace AlloyWithOutputCacheAttribute.Helpers
{
    public static class HtmlHelpers
    {
            ...
            bool requirePageTemplate = true)
        {
            ...
            var contentTrackingContext = helper.ViewContext.GetTrackingContext();
            ...
            contentTrackingContext.AddChildrenListingDependency(rootLink);
            var menuItems = contentLoader.GetChildren<PageData>(rootLink)
                .FilterForDisplay(requirePageTemplate, requireVisibleInMenu)
                .Select(x => CreateMenuItem(helper, x, currentContentLink, pagePath, contentLoader, filter))
                .ToList();

            if(includeRoot)
            {
                menuItems.Insert(0, CreateMenuItem(helper, contentLoader.Get<PageData>(rootLink), currentContentLink, pagePath, contentLoader, filter));
            }
        ...
        private static MenuItem CreateMenuItem(HtmlHelper helper, PageData page, ContentReference currentContentLink, List<ContentReference> pagePath, IContentLoader contentLoader, Func<IEnumerable<PageData>, IEnumerable<PageData>> filter)
        {
            helper.ViewContext.TrackContent(page);
